...
def adaptor_pairwise_three(param_list):
    return [tf.expand_dims(param, axis=1) for param in param_list]
...
def adaptor_P_pairwise(P):
    # P_gt is BxKxN'x3, making it BxKxKxN'x3
    # return tf.tile(tf.expand_dims(P_gt, axis=2), [1, 1, tf.shape(P_gt)[1], 1, 1])
    return tf.expand_dims(tf.expand_dims(tf.expand_dims(P, axis=1), axis=2), axis=2)
