...
def create_single_batch(cur_data, cur_group, cur_seg, n_max_instances, noisy, fixed_order=False, check_only=False, shuffle=True):
    '''
...
#use iterator
    ...
    P_gt.extend(
        [np.zeros(dtype=float, shape=[n_gt_points_per_instance, 3]) for _ in range(n_max_instances - n_instances)])
...
class Dataset:
    ...
    def fetch_data_at_file_index(self, i):
        assert not self.first_iteration_finished
        path = self.hdf5_file_list[i]  #modify
        name = os.path.splitext(os.path.basename(path))[0]
        with h5py.File(path, 'r') as handle:
            cur_data, cur_group, _, cur_seg = loadDataFile_with_groupseglabel_stanfordindoor(handle)
            data = create_single_batch(cur_data, cur_group, cur_seg, self.n_max_instances, noisy=self.noisy, fixed_order=self.fixed_order, shuffle=not self.fixed_order)
            #data = create_unit_data_from_hdf5(handle, self.n_max_instances, noisy=self.noisy, fixed_order=self.fixed_order, shuffle=not self.fixed_order)
            assert data is not None # assume data are all clean
